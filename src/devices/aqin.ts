...
export class aqin {
	...
	createAccessory(device: any, uuid: string, aqinSensor: PlatformAccessory) {
		...
		if(!airSensor){
			...
			if(device.lastData.aqi_pm25_aqin >300) {aqi=this.platform.Characteristic.AirQuality.POOR}
			else if(device.lastData.aqi_pm25_aqin >200) {aqi=this.platform.Characteristic.AirQuality.POOR}
			else if(device.lastData.aqi_pm25_aqin >150) {aqi=this.platform.Characteristic.AirQuality.INFERIOR}
			else if(device.lastData.aqi_pm25_aqin >100) {aqi=this.platform.Characteristic.AirQuality.FAIR}
			else if(device.lastData.aqi_pm25_aqin >50) {aqi=this.platform.Characteristic.AirQuality.GOOD}
			else if(device.lastData.aqi_pm25_aqin >0) {aqi=this.platform.Characteristic.AirQuality.EXCELLENT}
	...
	async getStatusAir(sensorStatus: Service): Promise<CharacteristicValue> {
		if (sensorStatus.getCharacteristic(this.platform.Characteristic.StatusFault).value == this.platform.Characteristic.StatusFault.GENERAL_FAULT) {
			...
			let currentValue: any = sensorStatus.getCharacteristic(this.platform.Characteristic.AirQuality).value
			return currentValue
		}
	}
	...
	async getStatusCo2(sensorStatus: Service): Promise<CharacteristicValue> {
		if (sensorStatus.getCharacteristic(this.platform.Characteristic.StatusFault).value == this.platform.Characteristic.StatusFault.GENERAL_FAULT) {
			...
			let currentValue: any = sensorStatus.getCharacteristic(this.platform.Characteristic.CarbonDioxideDetected).value
			return currentValue
		}
	}
